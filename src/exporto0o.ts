...
export async function exportToOo(
  ...
  const baseArguments = [
    `-f ${variables.fromFormat}`,
    '--citeproc',
    `--resource-path="${variables.currentDir}"`,
    `--resource-path="${variables.attachmentFolderPath}"`,
    `--resource-path="${variables.vaultDir}"`,
    `--lua-filter="${variables.luaDir}/obsidian_docx.lua"`,
    (variables.options as any)?.referenceDocx ? `--reference-doc="${(variables.options as any).referenceDocx}"` : '',
    `-o "${variables.outputPath}"`,
    '-t docx'
  ].join(' ');

  const cmdTpl =
    setting.type === 'pandoc'
      ? `${pandocPath} "\${currentPath}" ${baseArguments} ${setting.arguments ?? ''} ${setting.customArguments ?? ''} ${extraArguments ?? ''}`
      : setting.command;
